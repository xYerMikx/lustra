...
export function getWebAnalyticsConfig(): PublicAnalyticsConfig {
  const metrikaId = parseMetrikaCounterId(
    process.env.NEXT_PUBLIC_YANDEX_METRIKA_ID,
  )
  const gaId = parseGaMeasurementId(process.env.NEXT_PUBLIC_GA_MEASUREMENT_ID)
...
export function getYandexSiteVerification(): string | null {
  return parseSiteVerificationToken(
